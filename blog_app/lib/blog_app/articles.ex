defmodule BlogApp.Articles do
  ...
  def list_articles_for_user(user_id, current_user_id)
    when user_id == current_user_id do # ログイン者と投稿者が一致したら、限定公開も見れる
      Article
      |> where([a], a.status in [1, 2])
      |> get_articles_for_user_by_query(user_id)
  end

  def list_articles_for_user(user_id, _cuurent_usre_id) do #公開記事のみ見れる
    Article
    |> where([a], a.status == 1)
    |> get_articles_for_user_by_query(user_id)
  end

  def list_draft_articles_for_user(user_id) do #下書き記事のみ取得
    Article
    |> where([a], a.status == 0)
    |> get_articles_for_user_by_query(user_id)
  end
